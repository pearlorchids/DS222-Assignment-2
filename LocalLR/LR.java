...
public class LR {
	...
	public void trainLR(String train) throws IOException{
		...
		for (int i = 1; i <= numEpochs; i++) {
			...
			while ((document = br.readLine()) != null) {
				...
				*/
				...
				/*for(int key :hashedFeatures.keySet())
				{
					System.out.println(hashedFeatures.get(key)+","+key);
				}
		...
		/*
		double [] valsW;
		for(int hashkey : wVectors.keySet())
		{
			valsW = wVectors.get(hashkey);
			for(int i=0;i<valsW.length;i++)
			{
				System.out.print(","+valsW[i]);
			}
			System.out.println();
		}*/
	}
