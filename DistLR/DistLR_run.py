...
if __name__ == '__main__':
  ...
  default_data_file = os.path.join(script_dir, 'trainsmallnew.csv')
  
  parser = argparse.ArgumentParser(description='Launches the JBosen Distributed LR application using SSH.')
  ...
  parser.add_argument('--eta', type=float, default=0.00185, help='Value of eta.')
  parser.add_argument('--lambda_', type=float, default=0.0001101, help='Value of lambda.')
  parser.add_argument('--num_iterations', type=int, default=20, help='Number of passes over the dataset to run.')
  parser.add_argument('--num_clocks_per_iteration', type=int, default=1, help='Number of clocks for each iteration.')
  parser.add_argument('--staleness', type=int, default=4, help='Number of clocks for each iteration.')
  ...
  class_path = os.path.join(script_dir, 'build', 'libs', 'DistLR.jar')
  main_class = 'DistLR'
  ...
  def launch(client_id, ip):
	...
	'''
print("Starting instances of DistLR...")
